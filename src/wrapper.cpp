...
extern "C" {
  ...
  int open (char *pathname, int flags) {
    return graceopen (pathname, flags);
  }

  int close (int fd) {
    return graceclose (fd);
  }
#endif

  int read (int fd, void * buf, int count) {
    return graceread (fd, buf, count);
  }

  int write (int fd, void * buf, int count) {
    return gracewrite (fd, buf, count);
  }

  int lseek (int fd, int offset, int whence) {
    return gracelseek (fd, offset, whence);
  }

  int connect (int fd, struct sockaddr *serv_addr, socklen_t addrlen) {
    return graceconnect (fd, serv_addr, addrlen);
  }

  int accept (int fd, struct sockaddr *addr, socklen_t * addrlen) {
    return graceaccept (fd, addr, addrlen);
  }
}
